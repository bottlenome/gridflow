...
- 評価指標の固定
- 実験比較
- 教育用演習化（教員パートナーの責務 — gridflow は拡張性のみ提供）
...
- leaderboard
- team workspace
- grading support（教員パートナー責務 — AC-3）
- fault injection
- sensitivity sweep
...
### 3.4 将来の拡張
LLM 連携（scenario 雛形生成、失敗原因要約、結果要約等）は将来的な拡張として検討し得る。アーキテクチャドキュメントでは UC-10（LLM による実験指示）としてアーキテクチャ上の契約（CLI/API の構造化 I/O、Ubiquitous Language の一貫性）を定義済みだが、P0 段階での完全な実装は含めない。
...
| GridLAB-D | Docker コンテナ内で実行（環境安定後に追加） | Apple Silicon ネイティブビルドが不安定。公式 Docker イメージあり。マルチアーキ安定性の確認後に Connector 実装 |
