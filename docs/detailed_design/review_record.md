...
### 8.6 論点 6.6: Orchestrator のレイヤー帰属と責務分割（事後発見）

**問題:** Phase 0 結果レビュー（論点6.1〜6.5）の対応完了後、アーキテクチャドキュメント整合性チェックの結果、`docs/architecture/03_static_view.md` L301 と `05_view_mapping.md` L13 で **Orchestrator が Use Cases 層**と位置付けられていることが判明。一方、詳細設計 v0.7 までの記述・モジュールパス (`gridflow.infra.orchestrator`) は **Infra 層**前提だった。これは私（v0.7 修正）以前から存在した矛盾だが、論点6.2 で 03b → 03d への移設をして「より明確な不一致」になった。

**論点:** Orchestrator は責務的に「実験実行のビジネスロジック (UseCase)」と「Docker コンテナ操作 (Infra)」の両方を持つ。どちらのレイヤーに置くべきか自体に議論の余地がある。

**選択肢:**

| 案 | 内容 | Good | Bad |
|---|---|---|---|
| **A** | 詳細設計を UseCase 層に戻す（モジュールパスを `gridflow.usecase.orchestrator` に） | 修正範囲が詳細設計のみ／アーキテクチャ doc と一致 | Docker 操作という技術詳細が UseCase 層に染み出す／層責務違反 |
| **B** | アーキテクチャ doc を Infra 層に修正 | 既存モジュールパスを維持できる | アーキテクチャ doc の位置付けを変える＝根本方針の妥協／「実験実行制御はビジネスロジックではない」と認めることになる |
| **C** | 役割分割: Orchestrator(UseCase, ビジネスロジック) と OrchestratorRunner(Infra, Docker 操作) に分ける | Clean Architecture の層境界を完全に守れる／アーキテクチャ doc と詳細設計が両方正／将来 Docker 以外の Runner（k8s, ローカルプロセス, モック）を追加しやすい | クラス分割と詳細設計の大幅修正が必要／実装影響大 |

**ユーザ判断:** **C 案採択**。「妥協なき理想設計の原則」(`CLAUDE.md` §0) に基づき、層境界を完全に守ることを優先。

**設計（v0.8 確定）:**

| クラス | 層 | モジュール | 責務 |
|---|---|---|---|
| `Orchestrator` | UseCase | `gridflow.usecase.orchestrator` | 実験実行のビジネスロジック。Docker を一切知らない |
| `OrchestratorRunner` (Protocol) | UseCase | `gridflow.usecase.interfaces` | Orchestrator が依存する物理実行基盤の境界 |
| `ContainerOrchestratorRunner` | Infra | `gridflow.infra.orchestrator` | OrchestratorRunner の Docker 実装 |
| `ContainerManager` | Infra | `gridflow.infra.orchestrator` | Docker 操作の低レベル詳細 |
| `ExecutionPlan` | UseCase | `gridflow.usecase.orchestrator` | 純粋データクラス |
| `TimeSync` (設定) | UseCase | `gridflow.usecase.orchestrator` | 時間同期設定データ |
| `TimeSyncStrategy` (Protocol) | UseCase | `gridflow.usecase.interfaces` | 既存 |
| `OrchestratorDriven` | UseCase | `gridflow.usecase.orchestrator.timesync` | Connector Protocol のみ依存。技術詳細なし |
| `FederationDriven` | Infra | `gridflow.infra.orchestrator.timesync` | HELICSBroker 依存 |
| `HybridSync` | UseCase | `gridflow.usecase.orchestrator.timesync` | TimeSyncStrategy Protocol 経由で合成 |
| `SimulationTask` / `TaskResult` | UseCase | `gridflow.usecase.scheduling` | 既存 |

**修正ファイル:**
- `03b_usecase_classes.md`: §3.3 を UseCase Orchestrator として復活（v0.7 のコメントアウト削除＋新規記述）
- `03d_infra_classes.md`: §3.8 を Infra 部分のみに縮小（ContainerOrchestratorRunner / ContainerManager / FederationDriven）
- `02_module_structure.md`: `usecase/orchestrator/` ディレクトリツリー新設、`infra/orchestrator/` 縮小
- `architecture/03_static_view.md`: L300-301 レイヤー対応表更新
- `architecture/05_view_mapping.md`: L13-14 / L79-80 更新（Orchestrator 分割、ScenarioRegistry Protocol/実装分離も併せて）
- `architecture/09_review_record.md`: §6 として詳細設計フェーズからの逆流レビュー記録

**Phase 1 への持ち越し:**
- 旧 `gridflow.infra.orchestrator.Orchestrator` の単一クラスを上記分割に従って実装する
- `OrchestratorRunner` Protocol を `gridflow.usecase.interfaces` に新設
- `ContainerOrchestratorRunner` Infra 実装を新設（既存の Docker 操作を内包）

**ユーザ発言:** 「cでお願いします」（C 案＝クラス分割を選択）
