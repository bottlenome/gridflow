...
|---|---|---|
| 0.1 | 2026-04-03 | 初版作成 |
| 0.2 | 2026-04-04 | Phase 6 最終整合性反映（用語集追加、ドキュメント一覧更新） |
...
| DIP | Dependency Inversion Principle。依存性逆転の原則。上位モジュールは下位モジュールに依存せず、両者は抽象（Protocol）に依存する |
| CDL | Canonical Data Layer。ツールごとの独自フォーマットを吸収する共通データ表現 |
| py-dss-interface | OpenDSS エンジンの Python バインディングライブラリ |
| L1〜L4 カスタマイズレベル | Plugin API のカスタマイズ段階（L1:設定変更、L2:プラグイン開発、L3:パイプライン構成、L4:ソース改変） |
| GridflowError | gridflow 全例外の基底クラス。エラーコード E-xxxx を保持 |
...
| 03_class_design.md | 第3章 クラス設計 | 2026-04-03 | 2026-04-04 |
...
| appendix.md | 付録 | 2026-04-03 | 2026-04-04 |
