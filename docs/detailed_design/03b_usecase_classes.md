...
## 3.3 Orchestrator関連 ＝ → 03d へ移設（v0.7）

> **移設通知:** 本節で扱っていた Orchestrator / ExecutionPlan / ContainerManager / TimeSync / OrchestratorDriven / FederationDriven / HybridSync / SimulationTask / TaskResult は、`gridflow.infra.orchestrator` モジュール配下の Infra 層クラスである。02_module_structure.md 2.1 のレイヤー定義と整合させるため、設計記述を [03d_infra_classes.md §3.8](03d_infra_classes.md#38-orchestrator関連reqf002) に移した。
>
> なお `TimeSyncStrategy`（Protocol）と `OrchestratorDriven` を含む TimeSync 戦略は本来 UseCase 層の Protocol だが、`OrchestratorDriven` 等の実装は Infra 層 Orchestrator と密結合のため一括で 03d に集約した。Protocol と実装の依存方向は維持される（Protocol が UseCase、実装が Infra）。
>
> 詳細経緯は `review_record.md` 論点6.2 参照。


<!-- 以下 3.3.2〜3.3.7 は v0.7 で 03d_infra_classes.md §3.8 へ移設済み。本文は削除。 -->
