...
| 7 | E2E ループが 5 ステップ（0） | 6 ステップに拡張（「実験定義」を追加、「分析」を「評価」に改名） | Scenario Pack による実験定義が gridflow の中心概念であり、独立したステップとして明示する必要がある |
| 8 | 静的ビューのクラス図がサブシステム別 5 分割（計画書 6） | インターフェース境界中心の 4 分割に再構成 | 設計判断が表れるインターフェース境界を軸に整理した方がアーキテクチャの意図が伝わる |
