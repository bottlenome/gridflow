...
| 6 | LLM 連携は「将来的な拡張」で「初期スコープ外」（3.4） | UC-10 としてアーキテクチャ上の契約を定義。P0 での完全実装は含めないが、LLM Agent が操作しやすいインターフェース設計（QA-9）は P0 から組み込む | CON-3（1人+AI 開発）の生産性に直結。AS-4 と同様、設計で将来を阻害しない |
