...
sequenceDiagram
    ...
    participant PB as ExecutionPlanBuilder
    participant LCM as ConnectorLifecycleManager
    ...
    CLI->>Orch: run(scenario_pack_id, options)

    Orch->>Registry: load(scenario_pack_id)
    Registry-->>Orch: ScenarioPack

    Orch->>PB: build(ScenarioPack)
    PB->>PB: validate + generate ExecutionPlan
    PB-->>Orch: ExecutionPlan

    Orch->>LCM: initialize_all(plan)
    LCM->>Connector: initialize(config)
    Connector->>ExtSys: 接続確立
    ExtSys-->>Connector: ready

    Orch->>SE: execute_plan(plan, context)
    loop 各 ExecutionStep
        SE->>Logger: step_start(step_name)
        SE->>Connector: execute(step, context)
        Connector->>ExtSys: ツール固有の呼出し
        ExtSys-->>Connector: 生データ
        Connector->>Connector: DataTranslator で CDL 形式に変換
        Connector-->>SE: StepResult(CanonicalData)
        SE->>CDL: store_result(experiment_id, data)
        SE->>Logger: step_end(step_name, duration)
    end

    Orch->>LCM: teardown_all()
    Orch-->>CLI: RunResult
    CLI-->>User: 実行サマリ表示
```

> **静的ビューとの対応:**
> - Orchestrator → ExecutionPlanBuilder → ConnectorLifecycleManager → StepExecutor の委譲構造（3.2.1）
> - ConnectorInterface, CDL は 3.2.1 のインターフェース境界 ①②
> - `DataTranslator で CDL 形式に変換` は Anti-Corruption Layer（2.4.1, 3.1.5, インターフェース境界 ④）
...
sequenceDiagram
    ...
    Docker->>Orch: initialize()
    Orch->>Registry: connect()
    Orch->>LCM: health_check_all()
...
> - Installer（3.2.1）が初回起動検知を担い、Orchestrator が起動・終了を管理する
...
sequenceDiagram
    ...
    Inst->>Inst: detect_first_run()
    Inst->>Net: サンプル Scenario Pack ダウンロード
    Net-->>Inst: サンプルパック
    Inst->>Registry: register(sample_packs)

    Inst->>LCM: health_check_all()
    alt 全 Connector 正常
        Inst-->>User: セットアップ完了（所要時間, 次のステップ案内）
    else 一部 Connector 失敗
        Inst-->>User: 部分完了（正常/失敗の Connector 一覧, 修正手順）
    end
...
> **分析・設計判断:**
> - **初回起動検知**は Installer が担う（3.2.1）。2 回目以降の起動では UC-04 に直行し、サンプル DL 等は実行しない
...
sequenceDiagram
    ...
    alt アップデート
        ...
        User->>CLI: 承認
        CLI->>Mig: backup()
        Mig->>FS: 現バージョンのバックアップ作成
        CLI->>Docker: 新イメージ pull
        Docker->>Net: pull
        Net-->>Docker: 新イメージ

        CLI->>Mig: migrate(from=v1.2, to=v1.3)
        Mig->>Registry: migrate_packs(schema_diff)
        Mig->>CDL: migrate_results(schema_diff)

        alt マイグレーション成功
            Mig-->>CLI: 完了
            CLI-->>User: アップデート完了（v1.3）
        else マイグレーション失敗
            CLI->>Mig: rollback(handle)
            Mig->>FS: バックアップから復元
            Mig-->>CLI: ロールバック完了
...
> - いずれも UC-07 にはない「既存状態への配慮」が核心であり、Migrator（3.2.1）がマイグレーション・バックアップ・ロールバックを担う
