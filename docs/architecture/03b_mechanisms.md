...
| 判断 | 内容 | 理由 |
|---|---|---|
| **API ドキュメントはコードから自動生成** | インターフェース定義のドキュメントコメントから自動生成。手動管理しない | コードとドキュメントの乖離を防ぐ。ツールは言語選択（ADR-001）後に決定 |
...
| **アーキテクチャドキュメントは手動管理** | 本ドキュメント群（01〜04, 03b）は手動で維持 | アーキテクチャ判断はコードから自動生成できない |
