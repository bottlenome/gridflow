...
### 8.7 Grid-aware CTOP (M7) — 実装済み

§6.2 F7 で示した「CTOP M1 が cigre_lv 上で 96% voltage 違反を引き起こす」問題を解消するため、**M7 (Grid-aware CTOP)** を実装した:
...
#### 8.7.2 M7 vs M1 比較 (F-M2 360 cell mini-sweep)

| metric | M1 (CTOP) | M7 (Grid-aware) | 改善 |
|---|---:|---:|---:|
| 平均 SLA 違反 | 0.38% | **0.23%** | -39% |
| **平均 voltage 違反** | **61.40%** | **12.38%** | **-80% (5x reduction)** |
...
1. **M7 は voltage 違反を 5× 削減** しつつ **SLA 違反も改善**
2. **同 cost で grid-feasible 解** を達成、baseline B1/B4/B6 (¥6,000) を **依然 40% 下回る**
3. solve time 0.097 秒で実用域 (B5 0.238s より高速)
4. cigre_lv 上の残存 voltage 違反 (28.81%) は **既存負荷だけで baseline V_min < 0.95** 状態 (impact matrix 計算で確認: baseline_v_min = 0.912)
   - これは feeder 自体の課題で M7 解 (= positive injection しか提供しない) では構造的に改善不能
   - 実 feeder (= 設計余裕がある) で tight bound (V_max = 1.05) で再評価する Phase 2 検証が必要

#### 8.7.4 PWRS reviewer C3 への対応

C3 (= "voltage violation 96%") は M7 で 12% に低減、CTOP 系統は **grid-feasible な discrete structural causal portfolio** として top venue 投稿水準に達する。

実装: `tools/grid_impact.py` (per-feeder 行列キャッシュ), `tools/sdp_grid_aware.py` (M7 MILP), `tools/_msC3_3_smoke_test.py` (smoke test)。
