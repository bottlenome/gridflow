...
## 11. 自己訂正記録 — CLAUDE.md §0.1 / §0.5 違反と修正

本 ideation で犯した policy 違反と訂正:

| 違反 | 該当ルール | 訂正 |
|---|---|---|
| §9 初版で「(β) Rule 9 v2 を random anchor 化」を PO に仰いだ | §0.5.2 (技術判断は実装者が決める) | policy §2.5.2 を再読し Rule 7-9 関係が制約されていないことを確認、自分で 🟢 と判定 |
| §9 初版で「(γ) policy 側を更新」を PO に仰いだ | 同上 | policy 更新不要と自分で判定 |
| §9 #2 を 🟡 (PO 検索依存) のまま停止 | §0.5.3 (探索不足なら自分で webfetch すべき) | PO 提供の CPCM を §4.5b で構造差分明示、🟢 に修正 |
| **§10 で P1 (実装範囲) / P2 (期間) / P3 (gridflow 結合) を 3 択 PO 仰ぎ** | **§0.1 (妥協なし理想設計)** | **理想 = 全 12 条件 / 必要時間投資 / gridflow Phase 2 API 活用 を一撃で確定すべきだった。妥協案を選択肢として提示すること自体が §0.1 違反** |

教訓:

1. **「ユーザーに聞きたくなった瞬間」は思考深度不足のシグナル** (§0.5.3)
2. **「妥協案を提示する」自体が §0.1 違反** — 理想がポリシーから一撃で導けるなら、選択肢を並べる時点で妥協を許容している
3. policy 文面 + 熟考で 90% 以上は技術判断 + 理想設計判断として処理可能。次回以降は自問テンプレ + §0.1 一撃適用を必ず通す

### 11.1 §10 の確定方針 (PO 確認なしで決定)

CLAUDE.md §0.1 一撃適用:

| 項目 | 確定 | 根拠 |
|---|---|---|
| **実装範囲** | **M1-M6 + B1-B6 全 12 条件** | 妥協 subset を選ぶ理由がない。各条件は §8 で 1 主張に対応 |
| **期間** | **必要なだけ投資** | 期間制約は実装後に見える技術見積。事前 cap は §0.1 違反 |
| **gridflow 結合** | **Phase 2 API 活用** (SweepResult / packs / sensitivity) | 独立 script は既存資産の重複実装、§0.1 違反 |
