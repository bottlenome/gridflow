...
## F. Risk Register (Phase D の失敗モードと対処)

各 Phase で起きうる失敗を事前列挙。発生時の対処方針も明記。

### F.1 D-1 (voltage metric 二分解) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| baseline / dispatch の重畳が線形仮定で外れる | 低 | 中 | DistFlow 線形近似 (~5% 誤差) を許容、誤差を report に明記 |
| 既存の `voltage_violation_ratio` metric を使う test/sweep が break | 中 | 中 | 既存 metric は残し新 metric を**追加**するだけにする (= 後方互換) |

### F.2 D-2 (tight bound MILP) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| **全 cell が infeasible** (cigre_lv で M7 strict が解なし) | **高** | **高** | (1) D-3 (M8) で active 配置を MILP 化、(2) 失敗を honestly 報告し論文 §6 に「(feeder, SLA) feasibility envelope」として組込 |
| solve time 急増 (timeout) | 中 | 中 | timeLimit を 300 秒に設定、超過は infeasible として記録 |
| pulp/cbc が tight 制約で数値不安定 | 低 | 低 | bigM tuning、scaling check |

### F.3 D-3 (M8 active+standby joint MILP) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| MILP 変数数 2N で N=200 → 400 binary、N=1000 → 2000 binary | 中 | 中 | scale=200 までは OK、N=1000 以上は M8b-greedy フォールバック |
| big-M 線形化の M 値選択ミスで infeasibility 偽陽性 | 中 | 高 | M = max(cap)*K で十分大、テストで検証 |
| active 集合の選択が SLA violation を悪化させる corner case | 低 | 中 | active capacity floor を制約に追加 |

### F.4 D-4 (feasibility envelope) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| 27,648 cells (= 3×6×4×8×3×16) で実行時間 ~24 時間 | 高 | 中 | 段階的 sweep: D-4a で M8 のみ → D-4b で M7+B4+B5 を加える |
| envelope が "全部 infeasible" や "全部 feasible" で情報量ゼロ | 低 | 高 | D-1, D-2 結果を見て α/β 範囲を再調整 |

### F.5 D-5 (実データ取得) のリスク (= **最も重要**)

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| **CAISO API が 403/503** (前セッションで起きた) | 中 | 高 | (1) 別 endpoint を試す、(2) ftp/HTTP archive を直接 DL、(3) AEMO に切替 |
| AEMO PDF からの table extraction が失敗 (camelot/tabula) | 中 | 中 | hand transcription または直接 CSV (= AEMO Data Dashboard) を試す |
| Pecan Street registration が時間がかかる (= 数日〜数週間) | 高 | 中 | CAISO + AEMO で minimum 2 source を先に確保 |
| 実データの schema が想定と違う | 中 | 低 | loader を schema 合わせて修正、metadata.json も更新 |
| 実データで sweep 再実行すると **CTOP の cost 優位が消える** | 中 | 高 | これは正直に報告すべき finding。論文の主張を limitation に格下げ |

### F.6 D-6 (multi-scale) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| N=5000 で M1 (MILP) が timeout | **高 (= 期待される結果)** | 低 | これは Theorem 2 を実証する good finding。greedy が引き受ける |
| N=1000 で M7 grid-aware が timeout | 中 | 中 | sample bus 数を縮約 (= cluster 化) または greedy 化 |

### F.7 D-7 (倫理的書き換え) のリスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| 過剰な self-criticism で論文の novelty が消失 | 低 | 中 | "limitation を明示しつつ contribution は維持" のバランス語彙 |
| review_record の判定取消が PO に同意されない | 低 | 中 | PO に意図を確認、合意取得 |

### F.8 横断的リスク

| Risk | 発生確率 | Impact | 対処 |
|---|---|---|---|
| **CLAUDE.md §0.1 (妥協なし) 再違反**: bound を緩めたい誘惑 | **高** | **致命的** | 本書 §G アンチパターン参照、毎 commit で self-check |
| 計算リソース不足 (4-core local) | 中 | 中 | sweep を分割、夜間 background 実行 |
| 別タスクで context が切れる | 中 | 低 | 各 MS で smoke test + commit + push を必ず実行 |

---

## G. アンチパターン (= 絶対やってはいけないこと)

### G.1 「数値を見せかけで合格させる」hack

❌ **やってはいけない**:
- relaxed bound (V_max=1.10) を使って「合格」と判定
- voltage 違反 metric を有利な方だけ報告
- 60% 違反を 12% に下げて「5x 改善」と書く (= 12% 自体が deployable でない)

✅ **やる**:
- 真の運用基準 (V_max=1.05, 年<0.1% 違反) で判定
- metric を内訳分解して honest に報告
- infeasible なら infeasible と書く

### G.2 「合成データで claim を作る」

❌ **やってはいけない**:
- 「CTOP は 40% コスト削減」を合成データだけで主張
- demo fixture を「実データ準拠」と書いて実証扱い
- Pecan Street の "registration が必要" を理由に scope 外と主張

✅ **やる**:
- 少なくとも CAISO 1 source は実データで sweep
- 実データ結果と合成結果を **両方** 載せる
- 取得不能なら limitation として明記

### G.3 「先行研究を曖昧にする」

❌ **やってはいけない**:
- B5 = "金融 causal portfolio" として CPCM を引用しつつ、実装は PC 簡易版
- CPCM との 5 軸差分を実装で検証せず positioning のみで主張

✅ **やる**:
- B5 が CPCM の核要素 (PDE control / nonlinear filtering) を **含まない**
  ことを論文で明示
- CPCM full 実装は future work と明記

### G.4 「査読者の負担を軽くする」hack

❌ **やってはいけない**:
- 図表が薄いまま「reproducible だから OK」
- 反証 trace (C7 / C8) で都合が悪い結果を隠す

✅ **やる**:
- 全 figure に discussion paragraph 添付
- 反証結果は **隠さず** 報告、limitation で議論

### G.5 「自分で判断できる技術判断をユーザーに仰ぐ」

❌ **やってはいけない**:
- 「α/β どちらで進めるか」を仰ぐ (= 技術判断、自分で決める)
- 「strict v_max は 1.05 か 1.04 か」を仰ぐ

✅ **やる**:
- ANSI C84.1 で 1.05 と決まっているので 1.05 を採用 (= 自己判断)
- product judgment (= スコープ削減) のみ仰ぐ

### G.6 「commit を batch する」

❌ **やってはいけない**:
- Phase D-2 全実装を 1 commit で push
- smoke test なしで commit

✅ **やる**:
- 各 MS (= 半日〜1 日単位) で smoke test 付き commit + push
...
Phase D が成功しない場合の選択肢:

### H.1 Plan B-1: try11 を「conditional acceptance」で投稿

Phase D-1, D-2, D-7 のみ実装 (= 1 週間)、limitation を厚く書く戦略:

- 論文タイトル: "CTOP: A Causal-Trigger Orthogonal Portfolio for VPP Standby
  Design with Operational Voltage Limits — A Feasibility Study"
- abstract で **synthetic-only / single-feeder-class** を明示
- target venue を PWRS から **IEEE SmartGridComm (workshop)** または
  **Energies (mid-tier journal)** に切替

### H.2 Plan B-2: try12 で別問題に切替

`docs/mvp_problem_candidates.md` の候補 1 (Volt-VAR) または候補 3 (復旧順序)
に切替、try11 の知見を **methodology の踏み台** として活用:

- try11 の Rule 7-9 v2 適用例として残す
- ideation_record / theorems / dataset framework を保持
- 新 try12 で異なる問題で再挑戦

**長所**: 別問題なら voltage 違反問題が起きない可能性
**短所**: try11 で投資した実装の多くが直接活用できない

### H.3 Plan B-3: 学術投稿を諦め、industry whitepaper に格下げ

Phase D-2 が全 cell infeasible の場合、CTOP は **小規模 LV feeder では
原理的に不適用** という finding を industry whitepaper として公開:

- target: gridflow blog / arXiv preprint / Energy Central
- contribution: "negative result" として公開、別研究者の参考に

### H.4 判断基準

```
Phase D-1, D-2 完了時に以下で判断:

  M7 strict feasible cell rate >= 70% → Plan A (= Phase D 続行)
  M7 strict feasible cell rate 30-70% → D-3 (M8) 後に再評価
  M7 strict feasible cell rate < 30% → Plan B-1 (限定 venue) or B-2 (try12 切替)
```

---

## I. 引継ぎメッセージ (前セッション → 次セッション)

### I.1 直近で起きたこと

前セッションで実装したのは以下:

1. F-M2 multi-feeder framework (3 feeder, 200-DER pool, 8 trace, 16 method,
   1080 cells, 70 分実行)
2. Theorem 1-3 を `theorems.md` に確立
3. Dataset 機能を gridflow first-class として実装 (Domain types + 6 loaders +
   Registry + Bridge + 41 tests pass)
4. M7 grid-aware MILP を実装 (relaxed bound 下で 96% → 12% voltage 違反低減)
5. 60% / 12% 合格判定で review_record を「合格」と書いた (= **倫理的問題、
   D-7 で取消必須**)

### I.2 PO の最後の指摘 (= 本書作成の trigger)

> "C3を60%で合格するというのはひどいと思います．壊れすぎでしょ．．運用考えた
> 現実的な基準にしないと．C2取得できなかったということでしょうか？ちょっと違う
> コンテナに移動するので次にやることを詳細に記述をお願いします．"

= **(1) 12% 合格は不可、(2) C2 実データは取得できなかった、(3) 詳細手順書 (= 本書) 作成**
...
# 2. 必読ファイル (= 本書 §C) のうち最低限を読む (15 分)
cat /home/user/gridflow/CLAUDE.md
less test/mvp_try11/NEXT_STEPS.md  # § A〜§I を読む

# 3. 環境セットアップ確認 (= §D) (10 分)
PYTHONPATH=src .venv/bin/python -m pytest tests/dataset/ -q
cd test/mvp_try11
PYTHONPATH=/home/user/gridflow/src /home/user/gridflow/.venv/bin/python \
    -m tools._msC3_1_smoke_test

# 4. Phase D-1 (voltage metric 二分解) 着手
# = §3 詳細手順に従って実装、半日〜1 日で完了
```

### I.4 投資すべきマインドセット

- 「12% 合格」を取消したのは **prosocial behavior**。これを次セッションで
  ぐらつかせない。同じ過ちを繰り返さない。
- CLAUDE.md §0.1 (妥協なし) と §0.5.3 (自己判断) は **常に self-check**。
- 「ユーザーに聞きたい」と思った瞬間 = §0.5.3 自問テンプレを通す。
- 「制約を緩めたい」と思った瞬間 = アンチパターン §G.1 を読み返す。
...
| 日付 | 内容 |
|---|---|
| 2026-04-30 (初版) | chunk 1-7 で Phase D-1〜D-7 詳細手順を作成 |
| 2026-04-30 (改訂) | PO レビュー指摘を受け §A 背景・§B 用語集・§C 必読・§D 環境・§E 現在状態・§F リスク・§G アンチパターン・§H 代替パス・§I 引継ぎメッセージ を追加。コンテナ跨ぎでも full context が引き継がれるよう強化 |
